...
for(var i=0; i<drumBtn.length; i++){
  ...
  drumBtn[i].addEventListener("click", function btnAudio(){
...
function makeSound(key){
  switch(key){
    ...
    case "l": var kick= new Audio("sounds/kick-bass.mp3");
              kick.play();
              break;
    default: console.log("Refresh the page");

  }
}

//Assign Animation to Buttons

function addAnimation(currentKey){
  var activeBtn= document.querySelector("."+currentKey)
  activeBtn.classList.add("pressed");

  setTimeout(function(){activeBtn.classList.remove("pressed"), 150});
}
